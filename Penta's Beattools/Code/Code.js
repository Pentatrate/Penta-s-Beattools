const version = "5.0",
	...
	tools = [{ // textGenerator
		...
		name: "decoChecker",
		desc: "Returns the amount of unhidden decos in your level. May also hide no longer used decos or merge them to reduce deco ids (and thus, reduce lag)",
		constants: [
			{ name: "level", desc: "The level file that your variant uses", type: "json", required: true, newRow: true },
			{ name: "hideUnused", desc: "Whether to hide all decos after their last use automatically", type: "boolean", required: false, newRow: true },
			{ name: "mergeIds", desc: "Whether to merge no longer used deco ids with new deco ids", type: "boolean", required: false, showWhenParam: "hideUnused", showWhenValue: [true] }
		],
		before: () => {
			const events = (constants.level?.events !== undefined ? constants.level?.events : constants.level), decos = [], decoEvents = events.sort((a, b) => a.time - b.time).filter(event => event.type == "deco");
			let time = -(10 ** 10), mergedDecos = [], mergedDecos2 = [], hideCount = 0, avoidInfinity = [];
			function getDecoIndex(id) { return decos.indexOf(decos.filter(event => event.id == id)[0]); }
			function findParentOf(id) { return decos.indexOf(decos.filter(deco => deco.children.includes(id))[0]) }
			function updateDecoChildren(deco) {
				avoidInfinity.push(deco.id),
					deco.children.forEach(id => {
						decos[getDecoIndex(id)].lastUpdate + (decos[getDecoIndex(id)].lastDuration || 0) < deco.lastUpdate + (deco.lastDuration || 0) && (
							decos[getDecoIndex(id)].lastUpdate = deco.lastUpdate, decos[getDecoIndex(id)].lastDuration = deco.lastDuration, decos[getDecoIndex(id)].lastOrder = deco.lastOrder,
							!avoidInfinity.includes(decos[getDecoIndex(id)].id) && (updateDecoChildren(decos[getDecoIndex(id)]))
						)
					})
			}
			while (decoEvents.some(event => event.time > time)) {
				...
				decoNow.forEach(event => {
					getDecoIndex(event.id) == -1 && (constants.mergeIds && (events[events.indexOf(event)].hide = events[events.indexOf(event)].hide === true ? true : false), decos.push({ id: event.id, hide: false, sx: 1, sy: 1, firstUpdate: event.time, firstOrder: event.order, children: [], lastUpdate: -(10 ** 10) }));
					const deco = decos[getDecoIndex(event.id)], parentID = findParentOf(deco.id);
					(deco.lastUpdate + (deco.lastDuration || 0) < event.time + (event.duration || 0)) && (
						deco.lastUpdate = event.time, deco.lastDuration = event.duration, deco.lastOrder = event.order
					), deco.lastDirectUpdate = event.time,
						event.hide !== undefined && (deco.hide = event.hide),
						event.sx !== undefined && (deco.sx = event.sx),
						event.sy !== undefined && (deco.sy = event.sy),
						event.parentid !== undefined && (
							parentID != -1 && (
								decos[parentID].children.splice(decos[parentID].children.indexOf(deco.id))
							),
							getDecoIndex(event.parentid) != -1 && event.parentid != deco.id && (
								decos[getDecoIndex(event.parentid)].children.push(deco.id)
							)
						),
						avoidInfinity = [], updateDecoChildren(deco),
			...
			constants.hideUnused && (
				decos.forEach(deco => {
					!deco.hide && (events.push({
						time: deco.lastUpdate + (deco.lastDuration || 0), angle: 0, type: "deco", order: (deco.lastDuration || 0) == 0 ? (deco.lastOrder || 0) + 1 : undefined,
						id: deco.id, hide: true
					}), hideCount++);
				})
			);
			function renameDecoID(oldID, newID) {
				for (let i = 0; i < events.length; i++) {
					let event = events[i];
					if (event.type == "deco") {
						if (event.id === oldID + "") {
							event.id = "decoMerger_" + newID;
						}
						if (event.parentid === oldID + "") {
							event.parentid = "decoMerger_" + newID;
						}
					}
				}
			}
			constants.mergeIds && (
				decos.forEach(deco => {
					events.push({
						time: deco.lastUpdate + (deco.lastDuration || 0), angle: 0, type: "deco", order: (deco.lastDuration || 0) == 0 ? (deco.lastOrder || 0) + 1 : undefined,
						id: deco.id, sprite: "", parentid: "", rotationinfluence: 1, orbit: false,
						x: 300, y: 180, r: 0, sx: 1, sy: 1, ox: 0, oy: 0, kx: 0, ky: 0,
						drawLayer: "fg", drawOrder: "0", recolor: -1, outline: false, effectCanvas: false, effectCanvasRaw: false
					})
				}),
				avoidInfinity = [],
				decos.sort((a, b) => a.lastUpdate + (a.lastDuration || 0) - b.lastUpdate - (b.lastDuration || 0)).forEach((deco, i) => {
					let newID = mergedDecos2.reduce((prev, time, index) => {
						if (prev !== undefined) { return prev; }
						if (time < deco.firstUpdate) {
							console.log(-time + deco.firstUpdate)
							return index;
						}
						return undefined;
					}, undefined);
					if (newID === undefined) {
						newID = mergedDecos2.length;
						// console.log("New ID " + newID + " for " + deco.id)
					} else {
						// console.log("Merged " + deco.id + " with " + newID)
					}
					renameDecoID(deco.id, newID), mergedDecos[i] = newID, mergedDecos2[newID] = deco.lastUpdate + (deco.lastDuration || 0)
				})
			),
				resultDiv.innerText = "Visible decos:\n" + decos.filter(deco => !deco.hide && deco.sx != 0 && deco.sy != 0).map(deco => ["id:", deco.id, "sx:", deco.sx, "sy:", deco.sy, "lastUpdate:", deco.lastDirectUpdate].join(" ")).join("\n") + "\n\nUnhidden scale 0 decos:\n" + decos.filter(deco => !deco.hide && !(deco.sx != 0 && deco.sy != 0)).map(deco => ["id:", deco.id, "sx:", deco.sx, "sy:", deco.sy, "lastUpdate:", deco.lastDirectUpdate].join(" ")).join("\n"),
				constants.hideUnused && (resultDiv2.innerText = JSON.stringify(constants.level), console.log("Hidden " + hideCount + " events")),
				console.log("Amount of unhidden decos:", decos.filter(deco => !deco.hide).length, "/", decos.length + (constants.mergeIds ? " => " + (mergedDecos2.length) : ""));
		},
		after: () => { },
		functions: [],
		dontUseEvents: true
		...
		before: () => {
			...
			switch (constants.extra) {
				case "maxInaccuracies": resultDiv2.innerText = Math.floor((totalHits + mineBeats.length) * (1 - getFirstByName(grades, constants.grade).acc / 100) * 4) / 4; break;
				case "maxMisses": resultDiv2.innerText = Math.floor((totalHits + mineBeats.length) * (1 - getFirstByName(grades, constants.grade).acc / 100) - constants.barelies / 4); break;
				case "maxBarelies": resultDiv2.innerText = Math.floor((totalHits + mineBeats.length) * (1 - getFirstByName(grades, constants.grade).acc / 100) * 4 - constants.misses * 4); break;
				case "grade":
					let acc = ((totalHits + mineBeats.length - constants.misses - constants.barelies * 0.25) / (totalHits + mineBeats.length)) * 100;
					resultDiv2.innerText = [
						grades.sort((a, b) => a.acc - b.acc).reverse().reduce((prev, grade, i) => prev || ((acc >= grade.acc || i == grades.length - 1) && grade.name), false),
						" (",
						Math.floor(acc * 1e4) / 1e4,
						"%)"
					].join(""); break;
			}
		},
		...
		functions: [],
		dontUseEvents: true
	}, { // levelsPlayedAnalyser
		name: "levelsPlayedAnalyser",
		desc: "",
		constants: [
			{ name: "playedLevels", desc: "The playedlevels.json file", type: "json", required: true, newRow: true }
		],
		before: () => {
			constants.playedLevels = Object.entries(constants.playedLevels)
			const mostPlayed = constants.playedLevels.reduce((prev, level) => level[1].timesPlayed > prev ? level[1].timesPlayed : prev, 0);
			let playCounts = []; while (playCounts.length < mostPlayed) { playCounts.push(true) }
			resultDiv.innerText = "Total Individual Levels Played: " + constants.playedLevels.length + "\nTotal Level Plays: " + constants.playedLevels.reduce((prev, level) => prev + level[1].timesPlayed, 0) + "\n.\nMost Played:\n- " + constants.playedLevels.toSorted((a, b) => b[1].timesPlayed - a[1].timesPlayed).slice(0, 10).map(level => {
				const splitKey = level[0].split("_");
				return (splitKey[0] != "" ? splitKey[0] : "*Unnamed Level*") + (splitKey[1] != "" ? " by " + splitKey[1] : "") + (splitKey[2] != "" ? ", charted by " + splitKey[2] : "") + (splitKey.length == 4 ? ", variant: " + splitKey[3] : "") + ", times played: " + level[1].timesPlayed
			}).join("\n- ") + "\n.\nGrades:\n- " + grades.map((grade, i) => constants.playedLevels.reduce((prev, level) => prev + (level[1].pctGrade >= grade.acc && (i == 0 || level[1].pctGrade < grades[i - 1].acc) ? "|" : ""), "") + " (" + grade.name + ", " + constants.playedLevels.reduce((prev, level) => prev + (level[1].pctGrade >= grade.acc && (i == 0 || level[1].pctGrade < grades[i - 1].acc) ? 1 : 0), 0) + ")").join("\n- ") + "\n.\nLevel Plays:\n- " + playCounts.map((_, i) => constants.playedLevels.reduce((prev, level) => prev + (level[1].timesPlayed == i + 1 ? "|" : ""), "") + (constants.playedLevels.reduce((prev, level) => prev + (level[1].timesPlayed == i + 1 ? 1 : 0), 0) != 0 ? " (" + (i + 1) + ", " + constants.playedLevels.reduce((prev, level) => prev + (level[1].timesPlayed == i + 1 ? 1 : 0), 0) + ")" : "")).filter((v, i) => !(v == "" && !constants.playedLevels.some(level => level[1].timesPlayed == i + 2))).reverse().join("\n- ");
		},
		after: () => { },
		functions: [],
		dontUseEvents: true
